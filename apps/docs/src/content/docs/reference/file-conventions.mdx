...
In addition to routes, Cloudwerk recognizes special directories for background jobs, services, and image handling.

### app/queues/

Queue consumer definitions for processing background jobs.

<FileTree>
- app/queues/
  - email.ts         # → queues.email
  - analytics.ts     # → queues.analytics
  - orders-dlq.ts    # → queues.ordersDlq (dead letter queue)
</FileTree>
...
**Naming:** Filename becomes queue name (`email.ts` → `queues.email`)

### app/services/

Reusable service definitions with RPC support.

<FileTree>
- app/services/
  - email/
    - service.ts     # Required: → services.email
    - utils.ts       # Optional helpers
  - payments/
    - service.ts     # → services.payments
</FileTree>
...
**Naming:** Directory name becomes service name (`email/` → `services.email`)

### app/images/

Image definitions for Cloudflare Hosted Images.
...
**Naming:** Filename becomes image client name (`avatars.ts` → `images.avatars`)
