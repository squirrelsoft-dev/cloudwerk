...
<Steps>
1. Create a new D1 database using the Cloudwerk CLI:

   ```bash
   # Create D1 database and auto-configure wrangler.toml
...
2. If using wrangler directly, copy the database ID from the output and add it to `wrangler.toml`:
...
3. Your D1 database is now available via `db` from `@cloudwerk/core/bindings`.
</Steps>
...
# This creates: migrations/0001_create_users.sql
```
...
Alternatively, access D1 via `context.env.DB` in loader functions:
